...
export default function EditStoreModal({ open, onClose, onEdit, store }) {
  ...
  const handleSubmit = e => {
    ...
    onEdit({ id: store.id, name, email, address, ownerId });
    onClose();
  };
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg p-6 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex gap-2 justify-end mt-4">
            ...
            <button type="submit" className="px-4 py-2 rounded-2xl bg-blue-500 text-white" disabled={!nameValid || !addressValid || !emailValid || !ownerValid}>Update Store</button>
          </div>
        </form>
      </div>
    </div>
